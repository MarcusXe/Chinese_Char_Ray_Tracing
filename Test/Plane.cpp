	...
	if(mid != 0)//光线与曲面有交点
	{
			...
			&&Round(InPoint1.IntersectionPoint.z)>=Z_min&&Round(InPoint1.IntersectionPoint.z)<=Z_max)
		{
 			...
 			InPoint.pMaterial = this->pMaterial;
			{
				...
				{
					InPoint.pMaterial.SetAmbient(CRGB(100,0.0,0.0));//材质对环境光的反射率
					InPoint.pMaterial.SetDiffuse(CRGB(100,0.0,0.0));//材质对环境光和漫反射光的反射率相等
					InPoint.pMaterial.SetSpecular(CRGB(100,0.0,0.0));//材质对镜面反射光的反射率
					InPoint.pMaterial.SetEmit(CRGB(100,0.0,0.0));//材质自身发散的颜色
