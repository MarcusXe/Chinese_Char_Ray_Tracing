...
bool Rect::GetInterPoint(CRay ray, CInterPoint &InPoint) //获取 直线 与 面的交点
{
	...
	if (mid != 0)//光线与面有交点
	{
		...
		{	
				...
				InPoint.pMaterial.SetAmbient(CRGB(0.0, 55.0, 0.0));//材质对环境光的反射率
				InPoint.pMaterial.SetDiffuse(CRGB(0.0, 55.0, 0.0));//材质对环境光和漫反射光的反射率相等
				InPoint.pMaterial.SetSpecular(CRGB(0.0, 55.0, 0.0));//材质对镜面反射光的反射率
				InPoint.pMaterial.SetEmit(CRGB(0.0, 55.0, 0.0));//材质自身发散的颜色
